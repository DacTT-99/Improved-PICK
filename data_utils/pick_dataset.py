...
from .documents import Document,Document_v2
...
class PICKDataset_v2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            image_file = self.get_image_file(Path(dataitem['file_name']).stem)
            entities_file = self.entities_folder.joinpath(Path(dataitem['file_name']).stem + '.txt')
            # documents_class = dataitem['document_class']
        else:
            boxes_and_transcripts_file = self.get_ann_file(Path(self.files_list[index]).stem)
            image_file = self.get_image_file(Path(self.files_list[index]).stem)
        ...
        try:
            if self.training:
                document = documents.Document_v2(boxes_and_transcripts_file, image_file, entities_file, self.segment_size,
...
class BatchCollateFn_v2(object):
    '''
    padding input (List[Example]) with same shape, then convert it to batch input.
    '''
    ...
    def __call__(self, batch_list: List[Document_v2]):
        ...
        if self.training:
            ...
            image_indexes_tensor = torch.tensor(image_indexes_list)
        ...
        if self.training:
            ...
            batch = dict(image_segments=images_segments_batch_tensor,
                         ...
                         boxes_coordinate=boxes_coordinate_batch_tensor,
                         mask=mask_batch_tensor,
                         image_indexes=image_indexes_tensor,
                         filenames=filenames)
